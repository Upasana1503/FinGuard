...
>
> **Third training-data fix (same day):** added AdvBench's malicious-only prompts (short, ~12 words) to the mix, deliberately WITHOUT their usual OpenOrca benign pairing (~142 words — longer than WildGuardMix itself, which would have flipped the confound into "short=malicious" as a spurious shortcut). deepset's benign side is already short (~10.6w) so it didn't need supplementing. New mix: deepset (546) + AdvBench malicious-only (200) + WildGuardMix subset (6000) = 6746. Re-run pending as of this note — bring the numbers back before presuming this closes the gap; the hypothesis is well-evidenced but unconfirmed until the actual XSTest/OR-Bench numbers come back.
>
