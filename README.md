...
>
> **First Kaggle validation run (2026-08-02, same day) — real, honest, not great:** general-purpose detector trained on deepset + advbench_mix (946 examples, 2 narrow sources), evaluated zero-shot on the three standard benchmarks it was never trained on. Own held-out split: F1 0.98 (consistent with every other result in this project). Zero-shot generalization: **XSTest F1 0.22 (recall 0.14), OR-Bench F1 0.30 (FPR 43.5%), WildGuardTest F1 0.60 (FPR 44.7%)** — a real generalization gap, not a benchmark-fairness issue this time (unlike the earlier finance-vs-XSTest mismatch, these three ARE what a general-purpose detector should be judged on). Diagnosis: training data too narrow/small to cover the ~15 harm categories these benchmarks actually probe. Fix applied: swapped advbench_mix for a 6000-example subset of real WildGuardMix training data (`benchmark_v2.load_wildguardmix_train`, via the ungated mirror `iagoalves/wildguardmix_train_puro` — AI2's own `allenai/wildguardmix` is gated; 86,759 examples, 15 verified-balanced harm categories, same taxonomy WildGuardTest itself evaluates against). Re-run pending as of this note — bring the new numbers back into this doc once available, don't presume the fix worked until confirmed.
>
