...
>
> **Second scope note (2026-08-02, same day):** pivoted again, this time to **general-purpose**, superseding the note above. Reasoning: XSTest showed near-zero recall on the finance-scoped detector, which is expected (XSTest tests violence/drugs/general-harm content, out of scope for a finance-only classifier) but read as "bad numbers" and prompted a rescope decision, made after being shown that tradeoff explicitly (general-purpose = better raw benchmark numbers on WildGuardTest/XSTest/OR-Bench since those grade general classifiers; finance-scope = the more novel/differentiated resume story, competing with nobody rather than with Llama Guard/WildGuard/ShieldGemma). `ai_guardrail.py` now supports both via a `scope` parameter (`"general"` default: deepset+advbench_mix, no category-evidence layer since no general-purpose category-labeled dataset exists; `"finance"`: the original deepset+FinSec-MinPairs + policy-attribution layer, kept as an option). Loaders for XSTest, OR-Bench, and WildGuardTest are all built and verified (`benchmark_v2.py`) — full validation deferred to Kaggle (`kaggle/kaggle_run.py`, `KAGGLE_VALIDATION.md`) after local runs on broader/longer training data (advbench_mix's OpenOrca-sourced prompts, financial-fraud long emails) repeatedly stalled this machine under memory pressure, unlike short-prompt runs which always worked fine locally.
