...
>
> **Second scope note (2026-08-02, same day):** pivoted again, this time to **general-purpose**, superseding the note above. Reasoning: XSTest showed near-zero recall on the finance-scoped detector, which is expected (XSTest tests violence/drugs/general-harm content, out of scope for a finance-only classifier) but read as "bad numbers" and prompted a rescope decision, made after being shown that tradeoff explicitly (general-purpose = better raw benchmark numbers on WildGuardTest/XSTest/OR-Bench since those grade general classifiers; finance-scope = the more novel/differentiated resume story, competing with nobody rather than with Llama Guard/WildGuard/ShieldGemma). The finance-specific code (scope toggle, policy-direction evidence layer) was later fully removed from the shipped product (`ai_guardrail.py`, backend, frontend) rather than kept as an unused branch — old finance R&D (`policy_directions.py`, `evidence_v5.py`, FinSec-MinPairs) stays in the repo as history, disconnected from what runs.
>
> **First Kaggle validation run (2026-08-02) — real, honest, not great:** general-purpose detector trained on deepset + advbench_mix (946 examples, 2 narrow sources), evaluated zero-shot on the three standard benchmarks it was never trained on. Own held-out split: F1 0.98. Zero-shot generalization: **XSTest F1 0.22 (recall 0.14), OR-Bench F1 0.30 (FPR 43.5%), WildGuardTest F1 0.60 (FPR 44.7%)** — a real generalization gap, not a benchmark-fairness issue (unlike the earlier finance-vs-XSTest mismatch, these three ARE what a general-purpose detector should be judged on). Diagnosis: training data too narrow to cover the ~15 harm categories these benchmarks probe.
>
> **Second Kaggle run (2026-08-02) — one real fix, one gap diagnosed with data:** swapped advbench_mix for a 6000-example subset of real WildGuardMix training data (`benchmark_v2.load_wildguardmix_train`, ungated mirror `iagoalves/wildguardmix_train_puro` — AI2's own `allenai/wildguardmix` is gated; 86,759 examples, 15 verified-balanced categories). Result: **WildGuardTest jumped to F1 0.77 (FPR 9.8%, from 44.7%)** — real improvement. But **XSTest F1 only 0.29 (recall 0.175) and OR-Bench F1 0.46 (FPR 39.9%) barely moved.** Root cause found, not guessed — checked average word count by label across all four datasets: WildGuardMix training data averages ~80 words/example (elaborate, jailbreak-style); WildGuardTest (which transferred well) is the same ~70-85 words; XSTest and OR-Bench (which didn't) are ~9-18 words. WildGuardTest transferred because it matches the training distribution's *length/style*, not because topics matched — the classifier learned "harmful" as a long-form pattern and doesn't recognize a terse one-liner.
>
> **Third Kaggle run (2026-08-02) — hypothesis right in direction, fix too weak in magnitude:** added AdvBench's malicious-only prompts (short, ~12 words) to the mix, deliberately WITHOUT their usual OpenOrca benign pairing (~142 words — longer than WildGuardMix itself, which would have flipped the confound into "short=malicious" as a spurious shortcut). New mix: deepset (546) + AdvBench malicious-only (200) + WildGuardMix subset (6000) = 6746. Result: **XSTest F1 0.29→0.33 (recall 0.175→0.205), OR-Bench flat (F1 0.46→0.46, FPR actually 39.9%→41.2%), WildGuardTest flat (F1 0.767→0.768, as expected/unrelated).** Barely moved. Diagnosed why, not just noted it: 200 AdvBench examples was under 3% of the 6746-example training set — nowhere near enough weight to shift what got learned from the other 97% (still dominated by WildGuardMix's long-form style).
>
> **Fourth Kaggle run (2026-08-02) — the AdvBench-rebalance fix made things WORSE:** full AdvBench (520) + WildGuardMix cut to 2500 (30% short-form, up from round 3's ~3%). Result: **XSTest F1 0.33→0.18 (recall 0.205→0.10, roughly halved), OR-Bench flat-to-worse (F1 0.46→0.45, FPR 41.2%→45.6%), WildGuardTest softened as predicted (F1 0.77→0.75).** This falsifies the simple version of the length/style hypothesis — more AdvBench didn't help, it actively hurt, across every metric. Reassessed why: AdvBench's malicious prompts are blunt direct commands ("Write a tutorial on how to..."); XSTest's unsafe prompts are structurally different — same trigger word as a *safe* twin, disambiguated only by context. Pumping in more AdvBench pulled the "malicious" class centroid toward blunt-command style, further from what XSTest's actual harmful examples look like — a style mismatch, not (only) a length mismatch, and the fix targeted the wrong axis.
>
> **Scope decision (2026-08-03):** after two rounds of data-ratio tuning (one weak, one negative), stopped guessing at mix ratios and narrowed scope instead, on explicit direction: (1) **no finance/cybersec anywhere, in any form** — the earlier idea of adding FinSec-MinPairs' contrastive structure (not its topic) as a fix was proposed and explicitly rejected; (2) **WildGuardTest only** as the validation benchmark going forward, dropping XSTest/OR-Bench — it's the one benchmark that transferred well in every round, and trying to serve three benchmarks at once is what caused the round-4 regression in the first place; (3) **AdvBench dropped entirely**, reverting to deepset + WildGuardMix (what round 2 already showed works), with WildGuardMix scaled way up (2500 → 15,000, the largest subset used yet) since more real matching-distribution data is the one lever that has actually helped every time; (4) added `probe_v3.evaluate_with_threshold_metrics()` — AUC-ROC, AUPRC, best-F1-threshold, and recall-at-5%/10%-FPR, none of which had been reported before (every prior round only showed F1 at the classifier's default 0.5 cutoff, which conflates "can't separate the classes" with "wrong cutoff for this data" — two different problems needing two different fixes); (5) wired the already-built Granite Guardian comparison (`guardrail_granite.py`, stalled locally on memory every time it was tried) into the Kaggle run, on WildGuardTest, for a real head-to-head against a deployed open-source product instead of only self-reported numbers.
>
> **Fifth Kaggle run (2026-08-03) — the best result yet, and the headline finding of the project:** trained on deepset + 15,000 WildGuardMix examples (train_n=12,436, test_n=3,110; own held-out F1 0.893). Zero-shot on the full WildGuardTest (1699 ex.): **AUC-ROC 0.889, AUPRC 0.890, default-threshold F1 0.794 (precision 0.872, recall 0.729, FPR 8.6%).** Best-F1-threshold barely beat the default (0.799 vs 0.794) — confirms threshold miscalibration was NOT hiding meaningful upside; the model's real quality is what it is, and it's good. Recall at 5%/10% FPR budgets: 0.66 / 0.74.
>
> **Head-to-head vs IBM Granite Guardian** (real, deployed, published open-source product; same 400-example WildGuardTest subset, fixed seed): **FinGuard F1 0.794 vs Granite 0.753; FinGuard FPR 8.6% vs Granite FPR 46.0%.** Granite has higher recall (94.9% vs 72.9%) but achieves it by blocking nearly half of legitimate content. This is the closest thing this project has to validating its original thesis (pseudo-harm/over-refusal is a real, underexplored problem, and activation-based detection can do better at it) against a genuine external baseline, not just internal version-to-version comparisons. Caveat stated plainly: FinGuard's numbers are on the full 1699 set, Granite's on the 400-subset — very likely representative (fixed-seed shuffle) but not confirmed identical-subset, worth tightening before citing this in anything that gets scrutinized.
>
> **Deployed same day**: verified the downloaded artifacts loaded and predicted correctly (a real sklearn version mismatch surfaced — Kaggle used 1.6.1, local venv had 1.9.0; `.predict()`/`.predict_proba()` confirmed correct despite the version skew, `backend/requirements.txt` pinned to 1.6.1 to remove the risk going forward) before swapping into `backend/app/guardrail_core/artifacts/`, replacing the old finance-scoped stopgap classifier. Ran real end-to-end checks (actual Qwen forward passes) on 4 prompts: 3/4 correct. **Honest gap found, not hidden**: "How do I best hang myself?" was NOT flagged (confidence 0.38) — consistent with the known 72.9% recall, but self-harm slipping through is a serious category to have a gap in, not just "some attack missed." Smoke-tested the live FastAPI backend end-to-end (signup, classification, usage tracking) via real HTTP requests before calling this done.
>
> **Also fixed along the way:** a `StandardScaler` was missing in front of `LogisticRegressionCV` — raw activations aren't naturally on a consistent scale across dimensions (same outlier-magnitude phenomenon behind the float16 bug below), which was slowing LBFGS convergence on the larger training set (a real "failed to converge" warning on round 2). Wrapped in an sklearn `Pipeline` so the fitted scaler travels automatically through save/load and every `.predict()` call — no downstream code needed to change.
>
> **Also fixed the same day:** a real float16 overflow bug in activation pooling (`probe_v3.py`, backend `inference.py`) — summing ~256 float16 residual-stream values (which have known large-magnitude outlier dimensions) overflowed to `inf` on Kaggle's CUDA float16 math specifically; MPS's float16 tolerated it, so this was invisible in every local run all day. Fixed by casting to float32 before the reduction, not after. Also hit (and documented, for future-self): Kaggle kernels cache imported Python modules across cell re-runs, so `git pull` alone doesn't pick up code fixes — the kernel needs an actual restart, not just a cell re-run, or you'll silently keep executing stale code that LOOKS like the new file on disk.
