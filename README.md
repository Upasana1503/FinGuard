...
>
> **Fourth training-data fix (same day):** verified the full AdvBench malicious set is 520 examples (was only using 200), and cut `wildguardmix_samples` from 6000 to 2500. New mix: deepset (546) + full AdvBench (520) + WildGuardMix subset (2500) = 3566, with short-form now **29.9% of training** (verified via loader counts) instead of ~3%. Explicit tradeoff being made: less WildGuardMix volume could soften WildGuardTest's round-3 transfer (F1 0.77) — the next run needs to check that number too, not just assume XSTest/OR-Bench improving means the trade was free. Also: OR-Bench's `or-bench-hard-1k` split is specifically the 1000 safe prompts curated because they trick over-cautious models, so some FPR there reflects the benchmark doing its job, not purely a bug — don't expect this rebalance to fully zero it out. Re-run pending as of this note.
>
